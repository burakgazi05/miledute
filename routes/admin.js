...
router.get('/setup', (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Admin Setup</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#050505;color:#e0e0e0;font-family:sans-serif;height:100vh;display:flex;justify-content:center;align-items:center}.box{width:100%;max-width:380px;padding:40px 30px;text-align:center}h1{font-size:1.2rem;letter-spacing:4px;margin-bottom:30px;color:#b08d57}input{width:100%;padding:14px 18px;background:transparent;border:1px solid #222;color:#e0e0e0;font-size:.85rem;outline:none;margin-bottom:12px}input:focus{border-color:#b08d57}button{width:100%;padding:14px;background:transparent;border:1px solid #b08d57;color:#b08d57;font-size:.8rem;letter-spacing:2px;cursor:pointer;margin-top:8px}button:hover{background:#b08d57;color:#050505}#m{font-size:.75rem;min-height:18px;margin-top:12px}</style></head><body><div class="box"><h1>ADMIN SETUP</h1><form id="f"><input type="text" id="u" placeholder="Kullanici Adi" required><input type="password" id="p" placeholder="Sifre (min 8 karakter)" required minlength="8"><button type="submit">Olustur / Guncelle</button></form><p id="m"></p></div><script>document.getElementById('f').addEventListener('submit',function(e){e.preventDefault();var u=document.getElementById('u').value.trim(),p=document.getElementById('p').value,m=document.getElementById('m');if(!u||!p){m.textContent='Kullanici adi ve sifre gerekli.';m.style.color='#cf6e6e';return}fetch('/admin/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})}).then(function(r){return r.json()}).then(function(d){m.textContent=d.message;m.style.color=d.success?'#6ecf6e':'#cf6e6e'}).catch(function(){m.textContent='Baglanti hatasi.';m.style.color='#cf6e6e'})});</script></body></html>`);
});
